...
void ofApp::update() {
	...
	for (int x = -300; x <= 0; x += 300) {

		int i = 0;
		for (int y = -450; y <= 450; y += 300) {
			...
			this->setRingToMesh(this->face, this->frame, glm::vec3(i++ % 2 == 0 ? x : -x, y, 0), 0, 50, 10, rotation);
			for (auto radius = 60; radius <= 190; radius += 30) {

				glm::highp_mat4 rotation;
				auto noise_value = ofNoise(y, ofGetFrameNum() * 0.003 + radius * 0.0005);
				...
				this->setRingToMesh(this->face, this->frame, glm::vec3(i % 2 ? x : -x, y, 0), radius, 25, 25, rotation);
			}
		}
	}
}
